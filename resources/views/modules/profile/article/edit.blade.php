            ...
            <form class="needs-validation" novalidate="" action="{{ route('articles.update', ['id' => $article->id]) }}" method="post">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        @php
                            $listStatusBlockedModerator = \App\Article::getStatusListBlockedModerator();
                        @endphp

                        @if(array_has($listStatusBlockedModerator, $status))
                            <select class="form-control" id="status" name="status">
                                @if(isset($listStatusBlockedModerator[$status]))
                                    <option value="{{ $status }}" selected>{{ $listStatusBlockedModerator[$status] }}</option>
                                @endif
                            </select>
                        @else
                            <select class="form-control" id="status" name="status">
                                @foreach(\App\Article::getStatusListShowForUser() as $key => $value)
                                    @if($key == $status)
                                        <option value="{{ $key }}" selected>{{ $value }}</option>
                                    @else
                                        <option value="{{ $key }}">{{ $value }}</option>
